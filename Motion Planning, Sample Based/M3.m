...
function [path, path_found] = M3(robot, samples, adjacency, q_start, q_goal, link_radius, sphere_centers, sphere_radii)
    ...
    [neighborInd,~] = knnsearch(samples,[q_start;q_goal],'K',1); % FInds the nearest neighbors in samples for each configuration [q_start;q_goal]
    
    onInd = neighborInd(1,:);
    offInd = neighborInd(2,:);
    
    G = digraph(adjacency);

    [path,dist] = shortestpath(G,onInd,offInd);
